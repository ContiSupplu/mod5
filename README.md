...
- **The dog stays put** — the server orders it to sit for the duration and restores whatever
  it was doing before when you close the menu.
...
- **Headwear** — Crown of the Realm, Mage's Cap, or Knight's Helm.
- **Back gear** — Adventurer's Pack (with bedroll), Rescue Keg (St. Bernard style), or Royal Cape.
...
Hats and gear are saved with the dog and visible to everyone nearby — your dog keeps its
crown and backpack while running around after the makeover (great for the outro shot).
...
- Puppies wear everything too, at 55% scale.

## Tuning

All the "cinematography" constants (orbit distance, speed, framing offset, glide time) are at
the top of `client/ClientGroomingSession.java`. Every accessory offset/size lives in
`client/AccessoryRenderer.java` — plain numbers, safe to nudge.
